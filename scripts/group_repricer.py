...
from strategy import config, repricing_decisions, coherence_decision  # noqa: E402
...
def execute(client, market, side, amount, venue, reason, live, cfg):
    ...
    with st.locked_state(live) as s:
        ...
        if block:
            return block
        r = client.trade(market_id=market.id, side=side, amount=amount, venue=venue,
                         source=TRADE_SOURCE, skill_slug=SKILL_SLUG, reasoning=reason)
...
def exit_position(client, market, price, venue, reason, live):
    with st.locked_state(live) as s:
        pos = s["positions"].get(market.id)
        if not pos or pos["status"] != "open":
            return "no open position"
        r = client.trade(market_id=market.id, side="no", amount=pos["cost"], venue=venue,
                         source=TRADE_SOURCE, skill_slug=SKILL_SLUG, reasoning=reason)
        if not getattr(r, "success", False):
...
def run(venue, live):
    ...
    for letter, legs in sorted(groups.items()):
        ...
        for m, p, reason in exits:
            err = exit_position(client, m, p, venue, reason, live)
            print(f"  {'~ EXIT ' if not err else '! EXIT skipped: ' + err:40s} {m.question[:60]!r}")
        for m, p, reason in entries:
            err = execute(client, m, "yes", min(cfg["MAX_TRADE_USD"], cfg["MAX_TRADE_USD"]),
                          venue, reason, live, cfg)
